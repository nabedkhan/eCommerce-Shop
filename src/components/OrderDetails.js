...
import { Col, Container, Image, ListGroup, Media, Row } from 'react-bootstrap';
...
const OrderDetails = () => {
    ...
    const { name, email, address, phone, city, country, postalCode } = order.shippingAddress;

    const items = order.items.reduce((acc, item) => acc + (item.price * item.qty), 0);
